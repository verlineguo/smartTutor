...
def process_page_tfidf_and_cosine(page_text, language):
    ...
    page_text_cleaned = clean_text(page_text, language)

    if language.lower() == "japanese":
        page_text_cleaned = tokenize_japanese(page_text_cleaned)

    vectorizer = TfidfVectorizer(ngram_range=(1, 3), stop_words=stopwords_lang)
    tfidf_matrix = vectorizer.fit_transform([page_text_cleaned])
    ...
    chunk_size = 2000
    document_chunks = [page_text_cleaned[i:i + chunk_size]
                       for i in range(0, len(page_text_cleaned), chunk_size)]
    ...
    # Sort the results to return the top 10
    sorted_similarity = dict(
        sorted(similarity_results.items(), key=lambda item: item[1], reverse=True)[:10])

    return {
        "cosine_similarity": sorted_similarity
    }


def main_count_tfidf_per_page(file_path, language):
    # Membaca PDF
    import pdfplumber
    with pdfplumber.open(file_path) as pdf:
        pages = pdf.pages
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {executor.submit(process_page_tfidf_and_cosine, page.extract_text(), language): page_number
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                results[f"{page_number}"] = page_results
            except Exception as e:
                results[f"page_{page_number}"] = str(e)
