...
class QuestionController extends Controller
{
    public function uploadFile(Request $request)
    {
        ...
        $page = Http::attach('pdf', file_get_contents('/home/u486571172/domains/smart-tutor-fit.com/storage/app/public/file/' . $name), 'file.pdf', ['Content-Type' => 'pdf'])->post('http://91.108.110.58/count-page');
    ...
    {
        $Rawdata = Http::timeout(300)->post('http://91.108.110.58/cossine-similarity', [
    ...
    {
        $dataTable = DataTables::of($request->get('data'))->addIndexColumn()->make(true);
    ...
    public function translateDocument(Request $request)
    {
        ...
        if ($metadataExists) {
            return ResponseController::getResponse(['translation_metadata' => $topic->translation_metadata], 200, 'File sudah tersedia dalam bahasa yang diminta.');
        }
    ...
    public function calculateTfidf(Request $request)
    {
        ...
        $existingGramData = Gram::where('topic_guid', $topic->guid)->where('language', $request->get('language'))->exists();
        ...
        $response = Http::attach('pdf', Storage::disk('public')->get($translatedFile['path']), 'translated_file.pdf')
            ->timeout(900)
    ...
    public function calculateTfidfPage(Request $request)
    {
        ...
        $existingPageNouns = PageNoun::where('topic_guid', $topic->guid)->where('language', $request->get('language'))->exists();
    ...
    public function getPdfAnswersByQuestion($questionGuid)
    {
        try {
            ...
            $pdfAnswers = AnswerPDF::where('question_guid', $questionGuid)->orderBy('combined_score', 'desc')->get();
    ...
    public function saveQuestions(Request $request)
    {
        ...
        try {
            ...
            foreach ($questions as $questionData) {
                ...
                if (isset($questionData['answer_llama'])) {
                    AnswerLLM::create([
                        'question_guid' => $question->guid,
                        'answer' => $questionData['answer_llama'],
                        'source' => 'llama',
                    ]);
                }
            }
    ...
    public function generateData(Request $request)
    {
        ...
        // Kirim permintaan dengan data TF-IDF sederhana
        $generateResponse = Http::timeout(2000)
            ->attach('pdf', Storage::disk('public')->get($topic->file_path), 'file.pdf')
        ...
        Log::info('Flask API response:', ['data' => $responseData]);
    ...
    public function bulkUpdateThreshold(Request $request)
    {
        ...
        $updatedCount = Question::whereIn('guid', $request->guids)->update(['threshold' => $request->threshold]);
    ...
    public function showDataLanguage($guid, $language, Request $request)
{
    if ($request['user_id']) {
        ...
        $data = Question::leftJoin('answer_user', function($join) use ($userId) {
                $join->on('questions.guid', '=', 'answer_user.question_guid')
                     ->where('answer_user.user_id', '=', $userId);
            })
            ->where('questions.topic_guid', '=', $guid)
            ...
            ->select(
                ...
                'questions.weight',
                'questions.category',
                'questions.topic_guid',
                'questions.threshold',
                'questions.language',
                'questions.page',
                // Select answer_user columns with aliases
                'answer_user.guid as user_answer_guid',
                'answer_user.question_guid',
                'answer_user.user_id',
                'answer_user.answer',
                'answer_user.is_correct',
                'answer_user.evaluation_scores'
    ...
    public function showData($guid, Request $request)
    {
        if ($request['user_id']) {
            $userId = $request['user_id'];
            $data = Question::with([
                'user_answer' => function ($query) use ($userId) {
            ...
            $data = Question::where('topic_guid', '=', $guid)->orderByRaw('cast(page as unsigned) asc')->get();
        }
    ...
    public function setSelectedPdfAnswer(Request $request)
    {
        try {
            ...
            $answerExists = AnswerPDF::where('guid', $request->answer_pdf_guid)->where('question_guid', $request->question_guid)->exists();
            ...
            Question::where('guid', $request->question_guid)->update([
