...
class AnswerController extends Controller
{
    public function index($code, $guid, $id)
    {
        ...
        return view('answer.index', compact('token', 'code', 'name', 'id', 'guid', 'session'));
    }
    public function fill($guid)
    {
        ...
        $responseTopic = Http::withHeaders([
            'Authorization' => "Bearer " . $token,
            'Content-Type' => "application/json"
        ])->get(env("URL_API", "http://example.com") . '/api/v1/topic/deadline/' . $guid);
        $topic = json_decode($responseTopic, true);
        if ($topic['data']) {
            $name = $topic['data']['name'];
    ...
    public function answerResult($guid)
    {
        $session = new Session();
        $token = $session->get('access_token');
        $id = $session->get('id');
        ...
        $name = $topic['data']['name'];
        $code = $topic['data']['course_code'];
        return view('answer.user-answer-result', compact('token', 'guid', 'name', 'id', 'code', 'session'));
    }
}
